...
import { octokit_issuesAndPullRequests_mock } from '~tests/setup.ts'
...
describe('was_the_first_contribution()', () => {
  ...
  it('returns true if the closed PR is the first and only one', async () => {
    octokit_issuesAndPullRequests_mock.mockResolvedValue({ data: {items: [first_pr] }})
  ...
  it('returns true if the closed PR is the first of several', async () => {
    ...
    octokit_issuesAndPullRequests_mock.mockResolvedValue({ data: {items: [second_pr, first_pr] }})
  ...
  it('correctly handles issues', async () => {
    ...
    octokit_issuesAndPullRequests_mock.mockResolvedValue({ data: {items: [second_issue, first_issue] }})
  ...
  it('returns false if no contributions are found', async () => {
    octokit_issuesAndPullRequests_mock.mockResolvedValue({ data: {items: []} })
