...
describe('pull_request', () => {
  ...
  describe('.opened', () => {
    it('handles when a new pull request is opened', async () => {
      ...
      octokit_issuesAndPullRequests_mock.mockResolvedValue({ data: {items: [{ pull_request: {} }] }})
  ...
  describe('.closed', () => {
    ...
    it('handles when a pull request is merged', async () => {
      ...
      octokit_issuesAndPullRequests_mock.mockResolvedValue({
        data: {items: [{ number: 7, pull_request: {}, created_at: '2025-05-31T12:00:00Z' }]}
      })
    ...
    it('handles when a pull request is closed WITHOUT being merged', async () => {
      // This was the user's first and only PR
      octokit_issuesAndPullRequests_mock.mockResolvedValue({
        data: {items: [{ number: 4, pull_request: {}, created_at: '2025-04-20T12:00:00Z' }]}
      })
